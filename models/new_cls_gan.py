...
out_folder = './outputs/new_cls_gan_out/'
db_folder = './databases/new_cls_gan/imgs/'
...
class new_CLS_GAN():
    ...
    def train(self, niter=25, checkpoint=None):
        ...
        writer = SummaryWriter('./summaries/new_cls_gan')
        ...
        for epoch in range(self.current_epoch, niter):
            ...
            for i, data in enumerate(self.dataloader, 0):
                ...
                errD_real_img_fake_attr = 0.333*self.criterion(rf_output, label)
                ...
                errD_fake_img_real_attr = 0.333*self.criterion(fr_output, label)
                errD_fake_img_real_attr.backward(retain_graph=True)
                D_G_z_fr = fr_output.mean().item()

                # train with fake images, fake attributes
                # label = smooth_labels(label, strength=smooth_strength,
                #                      device=self.device, type=self.dtype)
                ff_output = self.netD(fake_img, fake_attr)
                errD_fake_img_fake_attr = 0.333 * self.criterion(ff_output, label)
                errD_fake_img_fake_attr.backward(retain_graph=True)
                D_G_z_ff = ff_output.mean().item()

                # Compute total loss for D and optimize
                errD = errD_real_img_real_attr + errD_fake_img_real_attr + errD_real_img_fake_attr + errD_fake_img_fake_attr
                ...
                #label = smooth_labels(label, strength=smooth_strength,
                #                      device=self.device, type=self.dtype)
                errG_real_attr = 0.5*self.criterion(fr_output, label)
                errG_real_attr.backward(retain_graph=True)

                label.fill_(fake_label)
                # label = smooth_labels(label, strength=smooth_strength,
                #                      device=self.device, type=self.dtype)
                errG_fake_attr = 0.5 * self.criterion(ff_output, label)
                errG_fake_attr.backward()

                errG = errG_real_attr + errG_fake_attr
                self.optimizerG.step()

                print(
                    '[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f D(x,y): %.4f D(x,y^): %.4f D(x^,y) %.4f D(x^,y^) %.4f'
                    % (epoch, niter, i, len(self.dataloader),

                       errD.item(), errG.item(), D_x, D_G_z_rf, D_G_z_fr, D_G_z_ff))
            ...
            self.save_checkpoint('%s/new_cls_gan_epoch_%d.pth' % (out_folder, epoch))
