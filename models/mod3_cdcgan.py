...
out_folder = './outputs/mod3_cdcgan_out/'
db_folder = './databases/mod3_cdcgan/imgs/'
...
class Generator(nn.Module):
    def __init__(self, ngpu, nz, ngf, nc):
        ...
        noise_layer = nn.Sequential(
            ...
            nn.ConvTranspose2d(nz, ngf * 15, 2, 1, 0, bias=False),
            nn.BatchNorm2d(ngf * 15),
            nn.ReLU(True)
            # state size. (ngf*15) x 2 x 2
        )
        ...
        output_layer = nn.Sequential(
            ...
            # state size. (ngf*15 + ngf*1) x 2 x 2
...
class Discriminator(nn.Module):
    def __init__(self, ngpu, ndf, nc):
        ...
        attribute_layer = nn.Sequential(
            ...
            Unsqueeze(),
            # input is 40 x 1 x 1
            nn.ConvTranspose2d(40, ndf * 1, 2, 1, 0, bias=True),
            nn.LeakyReLU(0.1, inplace=True)
            # state size. (ndf*1) x 2 x 2
        )

        output_layer = nn.Sequential(
            # input is image_layer(z) + attribute_layer(t)
            # state size. (ndf*16 + ndf*1) x 2 x 2
            nn.Conv2d(ndf * 16 + ndf * 1, 1, 2, 1, 0, bias=True),
            nn.Sigmoid()
        )
...
class mod3_cDCGAN():
    ...
    def train(self, niter=25, checkpoint=None):
        ...
        writer = SummaryWriter('./summaries/mod3_cdcgan')
        ...
        for epoch in range(self.current_epoch, niter):
            ...
            self.save_checkpoint('%s/mod3_cdcgan_epoch_%d.pth' % (out_folder, epoch))
