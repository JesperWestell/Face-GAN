...
class Generator(nn.Module):
    def __init__(self, ngpu, nz, ngf, nc):
        ...
        output_layer = nn.Sequential(
            ...
            # state size. (ngf*8 + ngf*4) x 2 x 2
            nn.ConvTranspose2d(ngf * 12, ngf * 8, 3, 1, 0, bias=False),
            nn.BatchNorm2d(ngf * 8),
            nn.LeakyReLU(0.1, inplace=True),
...
class mod_cDCGAN():
    ...
    def train(self, niter=25, checkpoint=None):
        ...
        writer = SummaryWriter('./summaries/mod_cdcgan')
        ...
        for epoch in range(self.current_epoch, niter):
            ...
            self.save_checkpoint('%s/mod_cdcgan_epoch_%d.pth' % (out_folder, epoch))
